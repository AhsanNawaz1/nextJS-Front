...
const Notification = ({ notification }: { notification: NotificationType }) => {
  ...
  const { profilePicture } = senderProfile;

  const profilePictureUrl =
    profilePicture.format?.thumbnail || profilePicture.url;

  return (
    <div className="flex h-16 items-center justify-start border-t bg-white px-4">
      <div className="mr-4">
        {notification.action === ActionType.COMMENT && <CommentIcon />}
        {notification.action === ActionType.DISCUSSION_REPLY && <CommentIcon />}
        {notification.action === ActionType.CONNECTION_REQUEST && <UserIcon />}
