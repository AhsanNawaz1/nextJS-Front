...
export default function ProfileDetails({ user, onboardingData }: IPageProps) {
  ...
  const [selectedSkills, setSelectedSkills] = useState(
    user?.profile?.skills ? user?.profile?.skills : null
  );

  const [selectedInterests, setSelectedInterests] = useState(
    user?.profile?.interests ? user?.profile?.interests : null
  );
    ...
    if (e.target.checked == true) {
      newValue =
        selectedInterests != null
          ? [...selectedInterests, e.target.value]
          : [e.target.value];
      setSelectedInterests(newValue);
    } else {
      newValue = selectedInterests?.filter((skill: string) => {
        return skill !== e.target.value;
      });
      setSelectedInterests(newValue);
    }
